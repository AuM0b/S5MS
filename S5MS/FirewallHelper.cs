	...
	class FirewallHelper
	{
		...
		public static void AddFirewallRule(INetFwRule rule)
		{
			...
			catch (Exception) 
			{
				...
				ConsoleHelper.WriteLine("* Error: Failed to add Firewall rule.", ConsoleColor.Red);
		...
		public static void RemoveFirewallRule(INetFwRule rule)
		{
			...
			{
				ConsoleHelper.WriteNewLine();
				ConsoleHelper.WriteLine(string.Format("* Error: Failed to remove Firewall rule '{0}'.", rule.Name), ConsoleColor.Red);
				ConsoleHelper.WriteLine("* Please restart the program to remove the rule from Firewall or remove it manually.", ConsoleColor.Red);
				ConsoleHelper.CloseApplication();
			}
		}
		...
		private static INetFwRule GenerateFirewallOutRule()
		{
			...
			firewallRule.Description = "This rule is generated by Steam Five Minutes Sharing. Remove it if the program has not done so.";
